...
const CN_CDN_CONFIGS = [
  ...
  {
    name: '国内CDN备用1',
    baseUrl: 'https://images.tmdb.org/t/p/', // 备用域名
    reliability: 0.7,
    speed: 0.7,
    region: 'cn',
    priority: 3
...
// 中国网络环境图片缓存
const cnImageCache = new LRUCache(200); // 增加图片缓存容量

// 智能CDN选择器 - 中国网络环境优化
function selectBestCDNForCN(size = 'w500', type = 'poster') {
  // 根据图片类型和大小选择最佳CDN
  if (size.includes('original') || size.includes('w1280')) {
    // 高清图片优先使用Cloudflare
    return CN_CDN_CONFIGS.find(cdn => cdn.name === 'Cloudflare代理') || CN_CDN_CONFIGS[0];
  }
  
  // 小图优先使用国内CDN
  if (size.includes('w342') || size.includes('w185')) {
    return CN_CDN_CONFIGS.find(cdn => cdn.name === '国内CDN备用1') || CN_CDN_CONFIGS[0];
  }
  
  // 中等图片随机选择，避免单一CDN压力过大
  const availableCDNs = CN_CDN_CONFIGS.filter(cdn => cdn.reliability > 0.6);
  return availableCDNs[Math.floor(Math.random() * availableCDNs.length)] || CN_CDN_CONFIGS[0];
}

// 智能图片URL生成器 - 中国网络环境优化
function createSmartImageUrlForCN(path, type = 'poster', size = 'w500') {
  if (!path) return '';
  
  const selectedCDN = selectBestCDNForCN(size, type);
  return `${selectedCDN.baseUrl}${size}${path}`;
}

// 图片降级策略 - 中国网络环境
function getImageFallbackUrls(path, type = 'poster', preferredSize = 'w500') {
  if (!path) return [];
  
  const fallbackSizes = {
    poster: ['w500', 'w342', 'w185'], // 海报降级顺序
    backdrop: ['w1280', 'w780', 'w500'] // 背景图降级顺序
  };
  
  const sizes = fallbackSizes[type] || ['w500', 'w342'];
  const urls = [];
  
  for (const size of sizes) {
    if (size === preferredSize) continue;
    const selectedCDN = selectBestCDNForCN(size, type);
    urls.push(`${selectedCDN.baseUrl}${size}${path}`);
  }
...
// 智能图片加载器 - 中国网络环境优化
async function loadImageWithCNFallback(primaryUrl, fallbackUrls = [], maxRetries = 2) {
  const urls = [primaryUrl, ...fallbackUrls];
  
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    for (const url of urls) {
      try {
        log(`[中国网络优化] 尝试加载: ${url}`, 'debug');
        const response = await Widget.http.get(url, {
          timeout: 5000, // 5秒超时
          headers: {
            'User-Agent': 'MovieListWidget/2.0-CN',
            'Accept': 'image/webp,image/apng,image/*,*/*;q=0.8'
          }
        });
        
        if (response && response.data) {
          log(`[中国网络优化] 成功加载: ${url}`, 'debug');
          return url;
        }
      } catch (error) {
        log(`[中国网络优化] 失败: ${url} - ${error.message}`, 'debug');
        continue;
      }
    }
  ...
  log(`[中国网络优化] 所有URL都失败`, 'warn');
  return null;
}

// 图片缓存管理 - 中国网络环境
function cacheImageUrlCN(key, url) {
  cnImageCache.set(key, { url, timestamp: Date.now() });
}

function getCachedImageUrlCN(key) {
  const cached = cnImageCache.get(key);
...
// 智能海报URL生成 - 中国网络环境优化
function createSmartPosterUrlForCN(item, preferredSize = 'w500') {
  if (!item.poster_path) return '';
  
  const cacheKey = `poster_${item.id}_${preferredSize}`;
  const cached = getCachedImageUrlCN(cacheKey);
  if (cached) {
    return cached;
  }
  
  const primaryUrl = createSmartImageUrlForCN(item.poster_path, 'poster', preferredSize);
  const fallbackUrls = getImageFallbackUrls(item.poster_path, 'poster', preferredSize);
  
  // 异步预加载和缓存
  loadImageWithCNFallback(primaryUrl, fallbackUrls).then(url => {
    if (url) {
      cacheImageUrlCN(cacheKey, url);
    }
  });
...
// 智能背景图URL生成 - 中国网络环境优化
function createSmartBackdropUrlForCN(item, preferredSize = 'w1280') {
  if (!item.backdrop_path) return '';
  
  const cacheKey = `backdrop_${item.id}_${preferredSize}`;
  const cached = getCachedImageUrlCN(cacheKey);
  if (cached) {
    return cached;
  }
  
  const primaryUrl = createSmartImageUrlForCN(item.backdrop_path, 'backdrop', preferredSize);
  const fallbackUrls = getImageFallbackUrls(item.backdrop_path, 'backdrop', preferredSize);
...
// 通用图片URL生成函数 - 中国网络环境优化
function generatePosterUrlCN(item, size = 'w500') {
  if (!item || !item.poster_path) return '';
  return createSmartPosterUrlForCN(item, size);
}

function generateBackdropUrlCN(item, size = 'w1280') {
  if (!item || !item.backdrop_path) return '';
  return createSmartBackdropUrlForCN(item, size);
}

// 批量图片预加载 - 中国网络环境优化
async function preloadImagesForCN(items, maxConcurrent = 3) {
  const imageUrls = [];
  
  // 收集所有图片URL
  for (const item of items) {
    if (item.poster_path) {
      const primaryUrl = createSmartImageUrlForCN(item.poster_path, 'poster', 'w500');
      const fallbackUrls = getImageFallbackUrls(item.poster_path, 'poster', 'w500');
      imageUrls.push({ primaryUrl, fallbackUrls, itemId: item.id });
    }
    
    if (item.backdrop_path) {
      const primaryUrl = createSmartImageUrlForCN(item.backdrop_path, 'backdrop', 'w1280');
      const fallbackUrls = getImageFallbackUrls(item.backdrop_path, 'backdrop', 'w1280');
      imageUrls.push({ primaryUrl, fallbackUrls, itemId: item.id });
    }
  }
  
  // 并发预加载
  const pool = getPromisePool(maxConcurrent);
  const tasks = imageUrls.map(({ primaryUrl, fallbackUrls }) => 
    () => loadImageWithCNFallback(primaryUrl, fallbackUrls)
  );
...
// 批量优化图片URL生成 - 中国网络环境
function optimizeImageUrlsCN(items) {
  return items.map(item => ({
    ...item,
    poster_url: generatePosterUrlCN(item, 'w500'),
    backdrop_url: generateBackdropUrlCN(item, 'w1280')
  }));
}

// 网络环境检测
function detectNetworkEnvironment() {
  // 这里可以根据实际情况实现更复杂的网络检测
  // 目前返回默认的中国网络环境配置
  return {
    region: 'cn',
    speed: 'medium',
    cdnPreference: 'cloudflare'
  };
}

// 自适应图片加载策略
function getAdaptiveImageStrategy() {
  const env = detectNetworkEnvironment();
  
  if (env.region === 'cn') {
    return {
      useCNOptimization: true,
...
// 中国网络环境性能监控
function logCNCacheStats() {
  log(`[中国网络优化] 图片缓存命中率: ${JSON.stringify(cnImageCache.stats())}`, 'info');
}
...
function createSmartPosterUrl(item, preferredSize = 'w500') {
  ...
  // 优先使用中国网络环境优化的海报URL生成器
  if (typeof createSmartPosterUrlForCN === 'function') {
    return createSmartPosterUrlForCN(item, preferredSize);
  }
