WidgetMetadata = {
  ...
  modules: [
    ...
    {
      title: "日本剧集",
      description: "日本电视剧和动画",
      requiresWebView: false,
      functionName: "japaneseTVShows",
      cacheDuration: 3600,
      params: [
        {
          name: "language",
...
if (typeof global !== 'undefined') {
  ...
  global.japaneseTVShows = japaneseTVShows; // 导出日本剧集函数
  global.detectAnimeContent = detectAnimeContent; // 导出动画检测函数
}
...
// 日本剧集数据获取 - 区分电视剧和动画
async function japaneseTVShows(params = {}) {
  const { 
    language = "zh-CN", 
    page = 1,
    content_type = "all", // all, anime, drama
    with_genres = "",
    sort_by = "popularity.desc",
    vote_average_gte = "6.0"
  } = params;
  ...
  try {
    ...
    if (content_type === "anime") {
      ...
      console.log(`[日本剧集] 筛选动画内容`);
    } else if (content_type === "drama") {
      // 电视剧类型筛选 - 排除动画
      queryParams.with_genres = "18"; // 剧情类型
      queryParams.without_genres = "16"; // 排除动画
      console.log(`[日本剧集] 筛选电视剧内容`);
    } else {
      // 全部内容
      if (with_genres && with_genres !== "") {
        queryParams.with_genres = with_genres;
      }
      console.log(`[日本剧集] 筛选全部内容`);
    }
    ...
    console.log(`[日本剧集] API请求参数:`, queryParams);
    ...
    console.log(`[日本剧集] 返回数据数量: ${res.results.length}`);
    ...
    return res.results
      .map(item => {
        ...
        // 智能判断是否为动画
        const isAnime = detectAnimeContent(item);
        
        // 添加日本剧集标识
        formattedItem.type = `japanese-${isAnime ? 'anime' : 'drama'}`;
        formattedItem.source = `日本${isAnime ? '动画' : '电视剧'}`;
        formattedItem.isJapanese = true;
        formattedItem.isAnime = isAnime;
        formattedItem.contentCategory = isAnime ? '动画' : '电视剧';
    ...
    performanceMonitor.logError();
    console.error("Error fetching Japanese TV shows:", error);
    return [];
  }
}

// 智能动画内容检测
function detectAnimeContent(item) {
  // 方法1: 通过类型ID判断
  if (item.genre_ids && item.genre_ids.includes(16)) {
    return true; // 16是动画类型ID
  }
  ...
  const animeKeywords = [
    'anime', 'アニメ', '动画', '动漫', 'anime', 'アニメーション',
    'manga', '漫画', 'マンガ', 'comic', 'コミック',
    'season', 'season 1', 'season 2', 'season 3',
    'episode', 'ep', '第', '季', '期'
  ];
  
  const dramaKeywords = [
    'drama', 'ドラマ', '电视剧', '剧集', '连续剧',
    'soap', 'soap opera', 'telenovela',
    'series', 'tv series', 'television'
  ];
  ...
  // 检查电视剧关键词
  const hasDramaKeywords = dramaKeywords.some(keyword => 
    title.includes(keyword) || overview.includes(keyword)
  );
  
  // 如果同时有动画和电视剧关键词，优先判断为动画
  if (hasAnimeKeywords) return true;
  if (hasDramaKeywords) return false;
  
  // 方法3: 通过原始语言判断
  if (item.original_language === 'ja' || item.original_language === 'ko') {
    // 日语和韩语内容更可能是动画
    return true;
  }
