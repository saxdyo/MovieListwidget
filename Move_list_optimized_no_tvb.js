...
async function tmdbDiscoverByNetwork(params = {}) {
  ...
  try {
    console.log(`[TMDB播出平台] 开始获取数据，参数:`, params);
    ...
    console.log(`[TMDB播出平台] 成功获取${processedData.length}条数据`);
    return processedData;
  } catch (error) {
    console.error(`[TMDB播出平台] 数据获取失败: ${error.message}`);
...
// ========== 其他功能函数（保持原有功能，删除TVB相关）==========

// 这里可以添加其他必要的函数，但避免重复定义

console.log("[优化完成] TVB函数已删除，脚本已优化");